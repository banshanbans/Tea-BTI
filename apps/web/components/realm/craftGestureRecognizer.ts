...
export function recognizeForwardPush(points: GesturePoint[], width: number, height: number): GestureRecognition {
  ...
  const forward = Math.abs(start.y - end.y);
  const drift = Math.abs(end.x - start.x);
  const matched = forward >= height * 0.18 && drift <= width * 0.7;
...
export function recognizeRolling(points: GesturePoint[], width: number, height: number): GestureRecognition {
  ...
  const reverseCount = reversals(points.map((point) => point.x), width * 0.025);
  const matched = reverseCount >= 2 && span >= width * 0.18 && drift <= height * 0.7;
  return { matched, score: matched ? clampScore(58 + reverseCount * 8 + span / width * 15 - drift / height * 8) : 0 };
}
...
export function recognizeCircle(points: GesturePoint[], width: number, height: number): GestureRecognition {
  ...
  if (boxWidth < width * 0.12 || boxHeight < height * 0.12) return { matched: false, score: 0 };
  ...
  const matched = pathLength >= Math.min(70, width * 0.55) && degrees >= 210 && aspect >= 0.35 && aspect <= 2.8 && closure <= diagonal * 0.72;
...
function reversals(values: number[], threshold: number): number {
  ...
  for (let index = 1; index < values.length; index += 1) {
    const delta = values[index] - anchor;
...
export function recognizePekoeMulti(first: GesturePoint[], second: GesturePoint[], width: number): GestureRecognition {
  ...
  const matched = duration >= 220 && centroidDrift <= width * 0.5 && reverseCount >= 1;
...
export function recognizePekoeSingle(points: GesturePoint[], height: number): GestureRecognition {
  ...
  const matched = reverseCount >= 2 && span >= height * 0.05 && span <= height * 0.55;
