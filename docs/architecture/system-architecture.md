...
入口在真正启动 Run 前先选择 `interactive / story`。文字稿由同一 Realm Definition 的七个审核章节和证据引用驱动，不请求设备权限；读到底并主动确认后通过独立接口点亮黔南和发放唯一白毫。阅读完成与互动完成分别记录，阅读不清除未完成 Run；行为结局始终优先于文字默认结局。

拨雾达到阈值后展示三个非强制探索点。探索点坐标、正文与证据 ID 来自 Catalog，点击不写场景进度，也不阻止直接进入采芽。
...
茶境不请求摄像头。麦克风只在用户主动选择吹气时请求；吹气、擦拭和直接开始是并列入口，蒸汽不再阻塞四手。超时或失败立即提供擦拭，音轨、AudioContext、方向监听、动画帧与定时器在切幕、隐藏或卸载时释放。屏幕手势只驱动互动，不声称识别现实芽叶、锅温、茶叶质量或专业制茶能力。
...
- `5.3—5.6 万芽头 / 500 克高级毛尖` 已改用农业农村部相关资料的可追溯口径，不再作为无来源事实债务。
